...
app.post("/api/medicines", async (req, res) => {
  ...
  if (!name || !benefits || price === undefined) {
    res.status(400).json({ error: "Missing fields" });
    return;
  }
  try {
    const med = new Medicine({ name, benefits, price, imageUrl: req.body.imageUrl, available: req.body.available !== undefined ? !!req.body.available : true });
    await med.save();
...
app.put("/api/medicines/:id", async (req, res) => {
  ...
  const { id } = req.params;
  const { name, benefits, price, imageUrl, available } = req.body;
  const patch = {};
...
app.post("/api/services", async (req, res) => {
  try {
    const { sellerId, firstName, lastName, serviceName, availableTime, location } = req.body;
...
app.put("/api/services/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { serviceName, availableTime, location } = req.body;
...
app.post("/api/products", async (req, res) => {
  try {
    const { sellerId, firstName, lastName, productName, imageUrl, location } = req.body;
